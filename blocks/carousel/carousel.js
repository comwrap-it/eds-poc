...
/**
 * Render a carousel item from Universal Editor data
 * @param {Object} item - The item data from Universal Editor
 * @returns {Element} - The rendered carousel item element
 */
function renderItemFromData(item) {
  const itemElement = document.createElement('div');
  itemElement.classList.add('carousel-item-columns-container');

  // Create image column
  const imageColumn = document.createElement('div');
  imageColumn.classList.add('carousel-item-column', 'carousel-item-image');
  
  if (item.image) {
    const picture = createOptimizedPicture(item.image, item.title || '', false);
    moveInstrumentation(item, 'image', picture);
    imageColumn.appendChild(picture);
  }
  
  // Create text column
  const textColumn = document.createElement('div');
  textColumn.classList.add('carousel-item-column', 'carousel-item-text');
  
  if (item.title) {
    const titleElement = document.createElement('h3');
    titleElement.innerHTML = item.title;
    moveInstrumentation(item, 'title', titleElement);
  ...
  if (item.description) {
    ...
    moveInstrumentation(item, 'description', descriptionElement);
    textColumn.appendChild(descriptionElement);
  }
  ...
  if (item.buttonText && item.buttonLink) {
    ...
    buttonElement.href = item.buttonLink;
    buttonElement.textContent = item.buttonText;
    buttonElement.classList.add('button', 'primary');
    moveInstrumentation(item, 'buttonText', buttonElement);
    
    buttonContainer.appendChild(buttonElement);
    textColumn.appendChild(buttonContainer);
  }
  
  itemElement.appendChild(imageColumn);
  itemElement.appendChild(textColumn);
...
class Carousel {
  ...
  */
  ...
  renderItem(item) {
    ...
    // Check if this is Universal Editor data or DOM element
    if (item.image || item.title || item.description) {
      // Universal Editor data
      return renderItemFromData(item);
    }
    
    // Existing DOM-based rendering for backward compatibility
